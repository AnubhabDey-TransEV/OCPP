...
from Chargers_to_CMS_Parser_2 import (
...
class CentralSystem:
    ...
    async def handle_charge_point(
        ...
        valkey_client.set(
            f"active_connections:{charge_point_id}", os.getpid()
        )  # Store worker/process ID with charge_point_id
        ...
        try:
            ...
            valkey_client.delete(
                f"active_connections:{charge_point_id}"
            )  # Remove from Valkey
            self.active_connections.pop(
                charge_point_id, None
            )  # Remove from self.active_connections
            self.charge_points[charge_point_id].online = (
                False  # Mark charger as offline
            )
...
async def get_configuration(request: GetConfigurationRequest):
    ...
    variable = (
        request.variable
    )  # Extract variable from the request body (optional)
...
async def query_charger_to_cms(request: ChargerToCMSQueryRequest):
    ...
    if request.filters:
        for column, value in request.filters.items():
            ...
            """
            column_exists = db.execute_sql(
                column_check_query, (column,)
            ).fetchone()[0]
    ...
    if request.limit:
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid limit format. Use 'start-end' format, e.g., '1-100'.",
            )
    else:
...
async def charger_analytics(request: ChargerAnalyticsRequest):
    ...
    start_time = request.start_time or datetime.min.replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    end_time = request.end_time or datetime.max.replace(
    ...
    for charger_id in charger_ids:
        ...
        if previous_timestamp and first_message:
            total_uptime_seconds = (
                previous_timestamp - first_message
            ).total_seconds()
        ...
        cursor = db.execute_sql(
            transaction_query, [start_time, end_time, charger_id]
        )
        total_transactions, total_electricity_used, total_time_occupied = (
            cursor.fetchone()
        )

        total_electricity_used = total_electricity_used or 0.0
        total_time_occupied = (
            total_time_occupied or 0
        ...
        """
        cursor = db.execute_sql(
            session_query, [start_time, end_time, charger_id]
        )
