...
class ChargePoint(CP):
    ...
    async def on_authorize(self, **kwargs):
        ...
        parser_c2c.parse_and_store_authorize(self.charger_id, id_token=id_token, status=status)
    ...
    async def on_transaction_event(self, **kwargs):
        ...
        if event_type == 'Started':
            ...
            parser_c2c.parse_and_store_transaction_event(self.charger_id, **kwargs)
        ...
        parser_ctc.parse_and_store_acknowledgment(self.charger_id, "TransactionEvent", "TransactionEvent", self.currdatetime(), connector_id=connector_id)
        return response
    ...
    @on('NotifyMonitoringReport')  # OCPP v2.0.1 equivalent of DiagnosticsStatusNotification
    async def on_notify_monitoring_report(self, **kwargs):
        ...
        parser_ctc.parse_and_store_acknowledgment(self.charger_id, "NotifyMonitoringReport", "NotifyMonitoringReport", self.currdatetime())
        return response
    ...
    async def remote_start_transaction(self, id_token, connector_id):
        logging.debug(f"Sending RemoteStartTransaction: id_token {id_token}, connector_id {connector_id}")
        ...
        parser_ctc.parse_and_store_remote_start_transaction(self.charger_id, id_token=id_token, connector_id=connector_id)
    ...
    async def change_availability(self, evse_id, operational_status):
        logging.debug(f"Sending ChangeAvailability: evse_id {evse_id}, operational_status {operational_status}")
        ...
        parser_ctc.parse_and_store_change_availability(self.charger_id, evse_id=evse_id, operational_status=operational_status, status=response.status)
    ...
    async def change_configuration(self, key, value):
        ...
        parser_ctc.parse_and_store_change_configuration(self.charger_id, key=key, value=value, status=response.set_variable_result[0].attribute_status)
    ...
    async def unlock_connector(self, evse_id):
        ...
        parser_ctc.parse_and_store_unlock_connector(self.charger_id, evse_id=evse_id, status=response.status)
    ...
    async def reserve_now(self, evse_id, expiry_date, id_token, reservation_id):
        ...
        parser_ctc.parse_and_store_reserve_now(self.charger_id, evse_id=evse_id, expiry_date=expiry_date, id_token=id_token, reservation_id=reservation_id)
        response = await self.call(request)
        parser_c2c.parse_and_store_reserve_now_response(self.charger_id, evse_id=evse_id, expiry_date=expiry_date, id_token=id_token, reservation_id=reservation_id, status=response.status)
        return response
