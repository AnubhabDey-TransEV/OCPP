from peewee import Model, CharField, MySQLDatabase, AutoField, DateTimeField, BooleanField, TextField
import requests
import json
...
def fetch_and_store_data():
    ...
    for charger in ChargerData.select():
        uid = charger.uid
        response = requests.post(url, headers=headers, json={"get_charger_id": uid})
        ...
        if response.status_code == 200:
            ...
            UserAndCharger.create(
                ...
                created_at=datetime.fromisoformat(user_details['createdAt'].replace("Z", "+00:00")),
                updated_at=datetime.fromisoformat(user_details['updatedAt'].replace("Z", "+00:00")),
                ...
                connector_total_capacity=charger_details['connector_total_capacity'],
                ...
                twenty_four_seven_open_status=charger_details['twenty_four_seven_open_status'] == "true",
                user_id=charger_details['userId']
            )
        else:
            print(f"Failed to fetch data for UID {uid}: {response.status_code}")
