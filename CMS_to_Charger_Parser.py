...
def add_column(column_name, column_type):
    try:
        if column_type == "string":
            db.execute_sql(
                f"ALTER TABLE CMS_to_Charger ADD COLUMN `{column_name}` TEXT"
            )
        elif column_type == "integer":
            db.execute_sql(
                f"ALTER TABLE CMS_to_Charger ADD COLUMN `{column_name}` INTEGER"
            )
        elif column_type == "float":
            db.execute_sql(
                f"ALTER TABLE CMS_to_Charger ADD COLUMN `{column_name}` FLOAT"
            )
        elif column_type == "datetime":
            db.execute_sql(
                f"ALTER TABLE CMS_to_Charger ADD COLUMN `{column_name}` DATETIME"
            )
